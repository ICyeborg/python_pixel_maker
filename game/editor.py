...
class Editor:
    def __init__(self, land_tiles, switch):
        ...
        # player
        CanvasObject((200, WINDOW_HEIGHT / 2), self.animations[0]['frames'], 0, self.origin,
                     [self.canvas_objects, self.foreground])

        # sky
        self.sky_handle = CanvasObject((WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2), [self.sky_handle_surf],
                                       1, self.origin, [self.canvas_objects, self.background])
            ...
            if cell in self.canvas_data:
                ...
                for name, side in NEIGHBOR_DIRECTIONS.items():
                    ...
                    if neighbor_cell in self.canvas_data:
                        # water top neighbors
                        if (self.canvas_data[neighbor_cell].has_water and
    ...
    def create_grid(self):
        ...
        for tile_pos, tile in self.canvas_data.items():
            ...
            if tile.coin:
                layers['coins'][(x + (TILE_SIZE // 2), y)] = tile.coin
            if tile.enemy:
    ...
    def object_drag(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONUP and self.object_drag_active:
            for sprite in self.canvas_objects:
                if sprite.selected:
                    sprite.release_drag(self.origin)
                    self.object_drag_active = False
    ...
    def draw_tile_lines(self):
        ...
        offset_vector = vector(x=self.origin.x - int(self.origin.x / TILE_SIZE) * TILE_SIZE,
        ...
        for col in range(cols + 1):
            ...
            pygame.draw.line(self.support_line_surface, LINE_COLOUR, (x, 0), (x, WINDOW_HEIGHT))
    ...
    def display_sky(self, dt):
        ...
        if y > 0:
            ...
            horizon_rect3 = pygame.Rect(0, y - 20, WINDOW_WIDTH, 1)
            pygame.draw.rect(self.display_surface, HORIZON_TOP_COLOUR, horizon_rect1)
            pygame.draw.rect(self.display_surface, HORIZON_TOP_COLOUR, horizon_rect2)
            pygame.draw.rect(self.display_surface, HORIZON_TOP_COLOUR, horizon_rect3)
        ...
        if 0 < y < WINDOW_HEIGHT:
            ...
            pygame.draw.rect(self.display_surface, SEA_COLOUR, sea_rect)
            pygame.draw.line(self.display_surface, HORIZON_COLOUR, (0, y), (WINDOW_WIDTH, y), 3)
        if y < 0:
            self.display_surface.fill(SEA_COLOUR)
